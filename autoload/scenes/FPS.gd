...
func update_text(force_update_mem:bool = false):
	...
	fps_label.text += mem+" / "+mem_peak+" [RAM]\n"
	...
	fps_label.text += vram+" / "+vram_peak+" [VRAM]\n"
	...
	mem_label.text += "Numerator: "+str(Conductor.numerator)+"\n"
	mem_label.text += "Denominator: "+str(Conductor.denominator)+"\n"
